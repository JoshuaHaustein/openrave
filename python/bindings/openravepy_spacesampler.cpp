...
protected:
    ...
    {
        if( samples.size() == 0 ) {
            return numpy::empty(boost::python::make_tuple(0), numpy::dtype::get_builtin<dReal>());
        }
        int dim = _pspacesampler->GetNumberOfValues();
        auto pyvalues = numpy::empty(boost::python::make_tuple(samples.size() / dim, dim), numpy::dtype::get_builtin<dReal>());
        memcpy(pyvalues.get_data(), &samples.at(0), samples.size() * sizeof(dReal));
        return pyvalues;
    }
    ...
    {
        if( samples.size() == 0 ) {
            return numpy::empty(boost::python::make_tuple(0), numpy::dtype::get_builtin<uint32_t>());
        }
        int dim = _pspacesampler->GetNumberOfValues();
        auto pyvalues = numpy::empty(boost::python::make_tuple(samples.size() / dim, dim), numpy::dtype::get_builtin<uint32_t>());
        memcpy(pyvalues.get_data(), &samples.at(0), samples.size() * sizeof(uint32_t));
        return pyvalues;
    }
};
