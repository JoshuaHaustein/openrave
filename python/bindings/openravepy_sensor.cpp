...
public:
        PyCameraSensorData(boost::shared_ptr<SensorBase::CameraGeomData const> pgeom, boost::shared_ptr<SensorBase::CameraSensorData> pdata) : PySensorData(pdata), intrinsics(pgeom->intrinsics)
        {
            ...
            {
                auto pyvalues = numpy::empty(boost::python::make_tuple(pgeom->height, pgeom->width, 3), numpy::dtype::get_builtin<uint8_t>());
        ...
        {
            {
                auto pyvalues = numpy::zeros(boost::python::make_tuple(pgeom->height, pgeom->width, 3), numpy::dtype::get_builtin<uint8_t>());
                imagedata = pyvalues;
            }
            {
