...
object search(ANNkd_tree& kdtree, object q, int k, double eps, bool priority = false)
{
    ...
    auto dims = boost::python::make_tuple(k);
    auto pydists = numpy::empty(dims, numpy::dtype::get_builtin<ANNdist>());
    BOOST_ASSERT(!!pydists);
    auto pyidx = numpy::empty(dims, numpy::dtype::get_builtin<int>());
    // if( !pyidx )
    //     Py_DECREF(pydists);
    BOOST_ASSERT(!!pyidx);
...
object search_array(ANNkd_tree& kdtree, object qarray, int k, double eps, bool priority = false)
{
    ...
    ANNpointManaged annq(kdtree.theDim());
    auto dims = boost::python::make_tuple(N, k);
    auto pydists = numpy::empty(dims, numpy::dtype::get_builtin<ANNdist>());
    BOOST_ASSERT(!!pydists);
    auto pyidx = numpy::empty(dims, numpy::dtype::get_builtin<int>());
    if( !pyidx ) {
...
object k_fixed_radius_search(ANNkd_tree& kdtree, object q, double sqRad, int k, double eps)
{
    ...
    auto dims = boost::python::make_tuple(min(k, kball));
    auto pydists = numpy::empty(dims, numpy::dtype::get_builtin<ANNdist>());
    BOOST_ASSERT(!!pydists);
    auto pyidx = numpy::empty(dims, numpy::dtype::get_builtin<int>());
    // if( !pyidx )
...
object k_fixed_radius_search_array(ANNkd_tree& kdtree, object qarray, double sqRad, int k, double eps)
{
    ...
    BOOST_ASSERT(len(qarray[0])==kdtree.theDim());
    ANNpointManaged annq(kdtree.theDim());
    auto dimsball = boost::python::make_tuple(N);
    auto pykball = numpy::empty(dimsball, numpy::dtype::get_builtin<int>());
    ...
    if( k <= 0 ) {
        for(int i = 0; i < N; ++i) {
            ...
            for (int c = 0; c < kdtree.theDim(); ++c)
                annq.pt[c] = extract<ANNcoord>(q[c]);
            pykball[i] = kdtree.annkFRSearch(annq.pt, sqRad, k, NULL, NULL, eps);
        }
        return boost::python::make_tuple(numpy::empty(boost::python::make_tuple(0), numpy::dtype::get_builtin<int>()),
                                         numpy::array(boost::python::list()),
                                         pykball);
    ...
    for(int i = 0; i < N; ++i) {
        ...
        for (int c = 0; c < kdtree.theDim(); ++c)
            annq.pt[c] = extract<ANNcoord>(q[c]);
        pykball[i] = kdtree.annkFRSearch(annq.pt, sqRad, k, &nn_idx[0], &dists[0], eps);
