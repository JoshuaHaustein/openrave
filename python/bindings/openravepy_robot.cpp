...
public:
        ...
        object FindIKSolutions(object oparam, int filteroptions, bool ikreturn=false, bool releasegil=false) const
        {
            ...
            if( ikreturn ) {
                ...
                unsigned int dofs = _pmanip->GetArmIndices().size();
                auto pysolutions = numpy::empty(boost::python::make_tuple(vsolutions.size(), dofs),
                                                numpy::dtype::get_builtin<dReal>());
                dReal* ppos = (dReal*)pysolutions.get_data();
                FOREACH(itsol,vsolutions) {
                    BOOST_ASSERT(itsol->size()==size_t(dofs));
