...
{
    auto pyvalues = numpy::empty(boost::python::make_tuple(7), numpy::dtype::get_builtin<dReal>());
    pyvalues[0] = t.rot.x; pyvalues[1] = t.rot.y; pyvalues[2] = t.rot.z; pyvalues[3] = t.rot.w;
    pyvalues[4] = t.trans.x; pyvalues[5] = t.trans.y; pyvalues[6] = t.trans.z;
    return pyvalues;
}
...
//        }
    ...
    object drawbox(object opos, object oextents, object ocolor=object(), object pytf=object())
    {
        RaveVector<float> vcolor(1,0.5,0.5,1);
        if( !IS_PYTHONOBJECT_NONE(ocolor) ) {
            vcolor = ExtractVector34(ocolor,1.0f);
        ...
        Transform tf;
        if ( !IS_PYTHONOBJECT_NONE(pytf)) {
            tf = ExtractTransform(pytf);
        }
        return toPyGraphHandle(_penv->drawbox(ExtractVector3(opos),ExtractVector3(oextents), vcolor, tf));
    }
...
BOOST_PYTHON_MEMBER_FUNCTION_OVERLOADS(drawbox_overloads, drawbox, 2, 4)
...
Because race conditions can pop up when trying to lock the openrave environment without releasing the GIL, if lockenv=True is specified, the system can try to safely lock the openrave environment without causing a deadlock with the python GIL and other threads.\n");
...

    {
        ...
        scope env = classenv
                    ...
                    .def("drawbox",&PyEnvironmentBase::drawbox,drawbox_overloads(args("pos","extents","color","tf"), DOXY_FN(EnvironmentBase,drawbox)))
