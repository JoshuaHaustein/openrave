...
private:
    ...
    {
        public:
            LinkAdjacentFilterCallback(KinBodyConstPtr pparent, const std::vector<int>& setadjacency) :
                OpenRAVEFilterCallback(), _pparent(pparent), _setadjacency(setadjacency) {
            }
    ...
    class btOpenraveDispatcher : public btCollisionDispatcher
    {
        public:
            ...
            virtual bool needsCollision(const btCollisionObject* co0, const btCollisionObject* co1) override
            {
            ...
            OpenRAVEFilterCallback* _poverlapfilt;
        private:
    ...
    {
        public:
            SetFilterScope(boost::shared_ptr<btOpenraveDispatcher> dispatcher, btOverlappingPairCache* paircallback, OpenRAVEFilterCallback* filter) :
                _dispatcher(dispatcher), _paircallback(paircallback) {
    ...
    bool CheckCollisionP(OpenRAVEFilterCallback* poverlapfilt, CollisionReportPtr report)
    {
        if( !!report ) {
...
public:
    BulletCollisionChecker(EnvironmentBasePtr penv, std::istream& sinput) :
        CollisionCheckerBase(penv), bulletspace(new BulletSpace(penv, GetCollisionInfo, false)), _options(0) {
    ...
    virtual bool CheckStandaloneSelfCollision(KinBodyConstPtr pbody, CollisionReportPtr report)
    {
        ...
        const std::vector<int>& non_adj_links = pbody->GetNonAdjacentLinks(adjacentoptions);
        LinkAdjacentFilterCallback linkadjacent(pbody, non_adj_links);
